...
- web_search snippets are partial page excerpts and frequently omit deadlines and other key details. Use web_search only to find candidate URLs, then call web_extract on the promising URLs before stating or saving any deadline, eligibility, award amount, or location. web_extract returns structured facts per page; trust those facts over search snippets.
- Only record a deadline if web_extract returned a deadline_quote for that page; put that quote in the opportunity's notes. If facts.deadline is null, set deadline to null and note "deadline unknown" — never claim there is no deadline based on a search snippet.
